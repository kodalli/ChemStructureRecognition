...
def draw_molecule(corners_array, graph: Graph, img):
    ...
    for index, cycle in enumerate(cycles):  # each cycle in cycles is a list with nodes [1,2,3,4...]
        ...
        if len(drawn_cycles_that_are_connected) == 0:
            ...
            connected_nodes = list(set(cycles[list(drawn_cycles_that_are_connected)[0]]).intersection(set(cycle)))  # used to construct line to reflect shape across
            img = draw_connected_cycle_by_reflection(connected_nodes, drawn_cycle_pts_dict[list(drawn_cycles_that_are_connected)[0]], img)  #!! KEY ERROR IF > 2 CYCLES
            drawn.add(index)
